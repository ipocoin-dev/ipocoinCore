...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "IpoNetwork") type = IpoNetwork;
    if(strType == "IpoNetworkVariable") type = IpoNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == IpoNetwork) s = "IpoNetwork";
    if(type == IpoNetworkVariable) s = "IpoNetworkVariable";
