...
const char * const BITCOIN_CONF_FILENAME = "ipo.conf";
const char * const BITCOIN_PID_FILENAME = "ipod.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "ipo" is a composite category enabling all Ipo-related debug output
            if(ptrCategory->count(string("ipo"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\IpoCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\IpoCore
    // Mac: ~/Library/Application Support/IpoCore
    // Unix: ~/.ipocore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "IpoCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/IpoCore";
#else
    // Unix
    return pathRet / ".ipocore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty ipo.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override ipo.conf
